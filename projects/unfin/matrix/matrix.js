...
var Matrix = function matrix() {
    ...
    var ui = document.getElementById('ui') || null;
    var width = null;
    var height = null;
    var spacing = 69;
    var nClosest = 4;
    ...
    function point(shape, size, size2, color) {
        pType = shape || "circle";
    ...
    function circle(pos, rad, opacity) {
        this.pos = pos || null;
        this.rad = rad || null;
        this.draw = function shapeCircleDraw() {
            ...
            if (opacity) ctx.fillStyle = "rgba(" + pColor + ", " + opacity + ")";
            else ctx.fillStyle = "rgba(" + pColor + ", " + this.active + ")";
            ctx.fill();
            ctx.closePath();
            ctx.restore();
        };
    ...
    function star(pos, rad, inset, p) {
        ...
        this.draw = function shapeStarDraw() {
            ...
            ctx.moveTo(0, 0 - this.rad) // moveTo this.pos.x & this.pos.y but on the outer radius/the first point of the star
            for (i = 0; i < this.points; i++) {
                ctx.rotate(Math.PI / this.points);
                ctx.lineTo(0, 0 - (this.rad * this.inset));
                ctx.rotate(Math.PI / this.points);
                ctx.lineTo(0, 0 - this.rad);
            }
    ...
    function heart(pos, rad) {
        ...
        this.draw = function shapeHeartDraw() {
            ...
            ctx.arc(0, 0, this.rad, 0, Math.PI, false);
            ctx.lineTo(this.rad, 0);
            ctx.arc(this.rad, -this.rad, this.rad, Math.PI * 90 / 180, Math.PI * 270 / 180, true);
